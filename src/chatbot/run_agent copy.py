...
from src.chatbot.other_agent import OtherAgent
...
def create_agent_graph(llm_client, checkpointer):
    ...
    other_agent = OtherAgent(llm_client)
    ...
    graph.add_conditional_edges(
        ...
        {END: END, "rag_agent": "rag_agent", "planning_agent": "planning_agent", "other_agent": "other_agent"}
    )
    ...
    graph.add_conditional_edges("other_agent", lambda state: END, {END: END})
...
def main():
    ...
    with SqliteSaver.from_conn_string(
        ...
        state: State = {
            ...
            "query_extract": None,
            "last_message": None,
            "user_memory": existing_memory,  # inject memory vào State
        }

        config = RunnableConfig(configurable={"thread_id": thread_id})
        turn_count = 0  # đếm số cặp hội thoại trong phiên này

        while not state.get("state_other", False):
            user_input = input("\n👤 Bạn: ")
            ...
            if turn_count % EXTRACT_EVERY_N_TURNS == 0:
                ...
                # Cập nhật lại user_memory trong state
