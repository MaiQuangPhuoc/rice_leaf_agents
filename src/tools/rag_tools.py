...
# from src.modules.rag.retrievers2 import _retriever as rag_retriever
...
@tool
def retrieve_tool(query: str, disease: str = None, keywords: str = None) -> str:
    """
    Tìm kiếm tài liệu về bệnh lúa từ cơ sở dữ liệu.
    Dùng khi cần thông tin về triệu chứng, nguyên nhân, cách phòng trị bệnh lúa.
    
    Args:
        query: Câu hỏi hoặc từ khóa tìm kiếm
        disease: Tên bệnh cụ thể (nếu đã biết)
        keywords: Từ khóa phụ, cách nhau bằng dấu phẩy
    """
    disease_list = [d.strip() for d in disease.split(",")] if disease else None
    ...
    for i, doc in enumerate(docs, 1):
        result += f"\n[{i}] Bệnh: {doc.metadata.get('disease', 'N/A')} | Topic: {doc.metadata.get('topic', 'N/A')}\n"
...
def ask_clarification_tool(question: str) -> str:
    """
    Hỏi lại người dùng khi thiếu thông tin để chẩn đoán chính xác.
    Dùng khi triệu chứng mô tả quá chung chung, không đủ để xác định bệnh.
    
    Args:
        question: Câu hỏi cụ thể cần hỏi lại người dùng
    """
    return f"CẦN_THÊM_THÔNG_TIN: {question}"
